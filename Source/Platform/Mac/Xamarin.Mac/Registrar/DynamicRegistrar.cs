...
{
    static IntPtrEqualityComparer __IntPtrEqualityComparer = new();
    static TypeEqualityComparer __TypeEqualityComparer = new();
	...
	public DynamicRegistrar()
	{
		...
		type_map = new Dictionary<IntPtr, ObjCType>(__IntPtrEqualityComparer);
		custom_type_map = new Dictionary<Type, object>(__TypeEqualityComparer);
	}
	...
	protected override bool ContainsPlatformReference(Assembly assembly)
	{
		...
		{
			if (assemblyName.Name == "Xamarin.Mac")
			{
				return true;
			}
		}
	...
	protected override IEnumerable<ProtocolMemberAttribute> GetProtocolMemberAttributes(Type type)
	{
		...
		{
			if (attrib is ProtocolMemberAttribute pmAttrib)
			{
	...
	protected override bool VerifyIsConstrainedToNSObject(Type type, out Type constrained_type)
	{
		...
		if (type.IsGenericTypeDefinition)
		{
			...
			for (int j = 0; j < genericArguments.Length; j++)
			{
				...
				array2[j] = constrained_type2;
			}
			constrained_type = type.MakeGenericType(array2);
			return flag;
		}
		return true;
	}
	...
	protected override void OnRegisterProtocol(ObjCType type)
	{
		...
		if (type.Protocols != null)
		{
			...
			foreach (ObjCType objCType in protocols)
			{
				...
				{
					ErrorHelper.Warning(4147, "Detected a protocol ({0}) inheriting from the JSExport protocol while using the dynamic registrar. It is not possible to export protocols to JavaScriptCore dynamically; the static registrar must be used (add '--registrar:static' to the additional mmp arguments in the project's Mac Build options to select the static registrar).", GetTypeFullName(type.Type));
				}
	...
	public void GetMethodDescriptionAndObject(Type type, IntPtr selector, bool is_static, IntPtr obj, ref IntPtr mthis, IntPtr desc)
	{
		...
		else
		{
			try
			{
				...
				mthis = ObjectWrapper.Convert(nSObject);
	...
	private bool RegisterMethod(ObjCMethod method)
	{
		...
		{
            Trampoline.Constructor => IntPtr.Zero,
			...
			//Trampoline.CopyWithZone1 => Method.CopyWithZone1, 
			//Trampoline.CopyWithZone2 => Method.CopyWithZone2, 
