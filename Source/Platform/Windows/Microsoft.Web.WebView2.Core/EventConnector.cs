...
internal class EventConnector
{
    ...
    public void addEventListener(string eventName, object JSHandler)
    {
        try
        {
            ...
            Delegate @delegate = _DelegatesMap.GetDelegate(id);
            ...
            if (_DelegatesMap.InsertDelegate(eventName, id, @delegate))
            {
                eventInfo.AddEventHandler(_originalHostObject, @delegate);
            }
        }
        catch (Exception ex)
        {
    ...
    public void removeEventListener(string eventName, object JSHandler)
    {
        try
        {
            Delegate @delegate = _DelegatesMap.RemoveDelegate(eventName, (JSHandler as ICoreWebView2PrivateRemoteObjectProxy).GetId());
            if ((object)@delegate != null)
            {
                CheckAndGetEventInfo(eventName).RemoveEventHandler(_originalHostObject, @delegate);
            }
        }
        catch (Exception ex)
        {
    ...
    private void RemoteObjectPassivated(object sender, object args)
    {
        try
        {
            ...
            foreach (string item in registeredEventNameById)
            {
                ...
                CheckAndGetEventInfo(item).RemoveEventHandler(_originalHostObject, @delegate);
            }
        }
        catch (Exception)
        {
    ...
    {
        EventInfo? @event = _originalHostObject.GetType().GetEvent(eventName, BindingFlags.Instance | BindingFlags.Public);
        if (@event == null)
        {
