...
{
    private class RawOptions : ICoreWebView2EnvironmentOptions, ICoreWebView2EnvironmentOptions2, ICoreWebView2EnvironmentOptions3, ICoreWebView2EnvironmentOptions4, ICoreWebView2EnvironmentOptions5, ICoreWebView2EnvironmentOptions6
    {
    ...
    //     the accept-languages HTTP header that WebView sends to websites. The intended
    //     locale value is in the format of BCP 47 Language Tags. More information can be
    //     found from [IETF BCP47](https://www.ietf.org/rfc/bcp/bcp47.html).
    ...
    //
    // Remarks:
    //     When AreBrowserExtensionsEnabled is set to true, new extensions can be added
    //     to user profile and used. AreBrowserExtensionsEnabled is default to be false,
    //     in this case, new extensions can't be installed, and already installed extension
    //     won't be available to use in user profile. If connecting to an already running
    //     environment with a different value for AreBrowserExtensionsEnabled property,
    //     it will fail with HRESULT_FROM_WIN32(ERROR_INVALID_STATE). See Microsoft.Web.WebView2.Core.CoreWebView2BrowserExtension
    //     for Extensions API details.
