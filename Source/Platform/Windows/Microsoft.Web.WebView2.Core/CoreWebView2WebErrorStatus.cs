...
public enum CoreWebView2WebErrorStatus
{
    ...
    //     HTTP response status code in this case is 401. See [status code reference](https://developer.mozilla.org/docs/Web/HTTP/Status).
    ValidAuthenticationCredentialsRequired,
    //
    // Summary:
    //     Indicates that user lacks proper authentication credentials for a proxy server.
    //     HTTP response status code in this case is 407. See [status code reference](https://developer.mozilla.org/docs/Web/HTTP/Status).
